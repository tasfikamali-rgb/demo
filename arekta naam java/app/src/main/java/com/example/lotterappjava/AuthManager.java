...
public class AuthManager {
    ...
    private static final String ADMIN_EMAIL = "zw@ulalberta.ca";
        ...
        db.collection("users").document(fbUser.getUid()).get()
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        ...
                        if (user != null) {
                            // Ensure admin role is persisted if email matches
                            if (ADMIN_EMAIL.equalsIgnoreCase(user.getEmail()) && !User.ROLE_ADMIN.equals(user.getRole())) {
    ...
    public void signUpWithEmail(Context context, String name, String email, String password, String role, AuthResultListener listener) {
        ...
        auth.createUserWithEmailAndPassword(email.trim(), password)
                .addOnCompleteListener(task -> {
                    if (task.isSuccessful() && task.getResult() != null && task.getResult().getUser() != null) {
                        FirebaseUser fbUser = task.getResult().getUser();
                        syncFirestoreUserAfterAuth(context, fbUser.getUid(), fbUser.getEmail(), name.trim(), finalRole, password, listener);
                    } else {
